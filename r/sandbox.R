...
for (i in 1:100000) {
  ...
  if (pbp_item$is_start_of_period == TRUE ||
    pbp_item$is_end_of_period == TRUE) {
    seconds_remaining <- 720.0
  }
...
fg_possession_length_summary <-
  as_tibble(as.data.frame(table(unlist(fg_possession_length)))) %>%
